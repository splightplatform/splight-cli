...
def Diff(new_arguments: Dict, old_arguments: Dict) -> Dict:
    ...
    for path, new_value in walk_dict(new_arguments):
        try:
            ...
            line = f"\t+ {'.'.join([str(key) for key in path])}: {new_value}"
            diff.append(line)
            continue

        old_value = get_dict_value(path, old_arguments)
        if new_value != old_value:
            line = f"\t~ {'.'.join([str(key) for key in path])}: {strike_text(old_value)} -> {new_value} "
            diff.append(line)
    ...
    for path, old_value in walk_dict(old_arguments):
        try:
            ...
            line = f"\t- {'.'.join([str(key) for key in path])}: {strike_text(old_value)}"
            diff.append(line)
